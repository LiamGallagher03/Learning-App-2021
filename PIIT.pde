...
public void theQuiz() {
  ...
  ws[1] = new rectWindow(150, 26, tempIon.formula);
  ws[2] = new rectWindow(a, 26, wIA.name);
  ws[3] = new rectWindow(b, 26, wIB.name);
  ws[4] = new rectWindow(c, 26, wIC.name);
  ws[5] = new rectWindow((yIndex * 100) + 250, 26, tempIon.name);
  for (rectWindow arr : ws) arr.draw();
}

public void theAnswer() {
  Ion tempIon = ions[randomIon];
  rectWindow var = new rectWindow((yIndex * 100) + 250, 26, tempIon.name);
  var.draw();
}

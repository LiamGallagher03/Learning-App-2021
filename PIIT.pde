...
public void arrayLoop() {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 8; j++) {
      windows[i][j] = new Window((i*100), (j*75), 210, ions[var].name);
      var++;
...
public void theQuiz() {
  ...
  rectWindow[] ws = new rectWindow[4];
  Window box = new Window(tempIon.formula);
  ws[0] = new rectWindow(a, 26, wIA.name);
  ws[1] = new rectWindow(b, 26, wIB.name);
  ws[2] = new rectWindow(c, 26, wIC.name);
  ws[3] = new rectWindow((yIndex * 100) + 200, 26, tempIon.name);
  for (rectWindow arr : ws) arr.draw();
  box.draw();
}
